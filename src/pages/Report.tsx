...
export default function Report() {
  ...
  const renderFieldValue = (carrier: string, field: FieldDefinition) => {
    ...
    if (field.type === 'boolean') {
      displayContent = value ? <Badge variant="default" className="gap-1 bg-green-500">
      ...
      displayContent = <div className="space-y-1">
      ...
      if (typeof value === 'object' && value !== null) {
        displayContent = <div className="space-y-1 text-sm">
            {Object.entries(value as Record<string, any>).map(([key, val]) => <div key={key} className="flex justify-between">
    ...
    return <div className={`p-3 rounded-lg border ${colorClasses[color || 'none']}`}>
        ...
        {note && <div className="mt-2 text-xs text-muted-foreground italic border-t pt-2">
            <MarkdownContent content={note} />
          </div>}
      </div>;
  };
  return <div className="min-h-screen bg-background print:bg-white">
      ...
      <main className="p-6 max-w-7xl mx-auto" ref={reportRef}>
        {/* Banner */}
        {bannerUrl && <img src={bannerUrl} alt={`Banner ${country}`} className="w-full h-48 object-cover rounded-xl mb-6 shadow-md print:h-32" />}
          ...
          <TabsContent value="dashboard" className="space-y-6">
            ...
            <Card className="bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
              ...
              <CardContent>
                ...
                // Most services
                const mostServices = [...carrierStats].sort((a, b) => Number(b.hasRedirect) + Number(b.hasPickup) + Number(b.hasSms) - (Number(a.hasRedirect) + Number(a.hasPickup) + Number(a.hasSms)))[0];
                        ...
                        <TooltipTrigger asChild>
                          <div className="bg-background rounded-lg p-4 border shadow-sm cursor-help hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3">
                              {bestOverall.logo ? <img src={bestOverall.logo} alt={bestOverall.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="bg-background rounded-lg p-4 border shadow-sm cursor-help hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3">
                              {bestAns.logo ? <img src={bestAns.logo} alt={bestAns.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-5 w-5 text-primary" />
                                </div>}
                              <div>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="bg-background rounded-lg p-4 border shadow-sm cursor-help hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3">
                              {lowestDev.logo ? <img src={lowestDev.logo} alt={lowestDev.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-5 w-5 text-primary" />
                                </div>}
                              <div>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="bg-background rounded-lg p-4 border shadow-sm cursor-help hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3">
                              {lowestSin.logo ? <img src={lowestSin.logo} alt={lowestSin.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-5 w-5 text-primary" />
                                </div>}
                              <div>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="bg-background rounded-lg p-4 border shadow-sm cursor-help hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3">
                              {mostServices.logo ? <img src={mostServices.logo} alt={mostServices.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-5 w-5 text-primary" />
                                </div>}
                              <div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                return <div className="space-y-3">
                      {ranked.map((carrier, index) => <Tooltip key={carrier.name}>
                          <TooltipTrigger asChild>
                            <div className={`flex items-center gap-4 p-3 rounded-lg border cursor-help hover:shadow-md transition-shadow ${index === 0 ? 'bg-yellow-50 dark:bg-yellow-950/30 border-yellow-200' : index === 1 ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-200' : index === 2 ? 'bg-orange-50 dark:bg-orange-950/30 border-orange-200' : 'bg-background'}`}>
                              ...
                              {carrier.logo ? <img src={carrier.logo} alt={carrier.name} className="w-10 h-10 object-contain rounded bg-white p-1 border" /> : <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-5 w-5 text-primary" />
                                </div>}
              ...
              <CardContent className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    return <tr key={carrier} className="border-b hover:bg-muted/30">
                          <td className="p-3">
                            <div className="flex items-center gap-2">
                              {logoUrl ? <img src={logoUrl} alt={carrier} className="w-8 h-8 object-contain rounded bg-white p-0.5 border" /> : <div className="w-8 h-8 rounded bg-primary/10 flex items-center justify-center">
                                  <Truck className="h-4 w-4 text-primary" />
                                </div>}
                          ...
                          <td className="p-3 text-center">
                            {hasRedirect ? <CheckCircle className="h-5 w-5 text-green-600 mx-auto" /> : <XCircle className="h-5 w-5 text-red-500 mx-auto" />}
                          </td>
                          <td className="p-3 text-center">
                            {hasPickup ? <CheckCircle className="h-5 w-5 text-green-600 mx-auto" /> : <XCircle className="h-5 w-5 text-red-500 mx-auto" />}
                          </td>
                          <td className="p-3 text-center">
                            {hasSms ? <CheckCircle className="h-5 w-5 text-green-600 mx-auto" /> : <XCircle className="h-5 w-5 text-red-500 mx-auto" />}
                          </td>
                          <td className="p-3 text-center">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    return <tr key={carrier} className="border-b hover:bg-muted/30">
                          ...
                          <td className="p-3 text-center">
                            <Badge variant={isGood ? 'default' : isWarning ? 'secondary' : 'destructive'} className="gap-1">
                              {isGood ? <><CheckCircle className="h-3 w-3" /> Excelente</> : isWarning ? <><AlertTriangle className="h-3 w-3" /> Aceptable</> : <><XCircle className="h-3 w-3" /> Revisar</>}
                            </Badge>
                          </td>
                        </tr>;
                  })}
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        ...
                        {carriers.map(carrier => {
                        const logoUrl = getCarrierLogo(carrier);
                        return <th key={carrier} className="text-center p-4 font-semibold min-w-[180px] bg-muted/50">
                              <div className="flex flex-col items-center gap-2">
                                {logoUrl ? <img src={logoUrl} alt={carrier} className="w-10 h-10 object-contain bg-white p-1 border rounded-xl" /> : <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
                                    <Truck className="h-5 w-5 text-primary" />
                                  </div>}
                                <span>{carrier}</span>
                              </div>
                    ...
                    <tbody>
                      {fields.map((field, fieldIndex) => <tr key={field.id} className={`border-b ${fieldIndex % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                        ...
                        if (field.type === 'boolean') {
                          ...
                          const numVal = parseFloat(String(value || 0));
                          const variant = getColorVariant(field.id, numVal);
                          displayContent = <div className="space-y-1 w-full max-w-[120px] mx-auto">
                                  <span className={`font-bold ${variant === 'success' ? 'text-green-600' : variant === 'warning' ? 'text-yellow-600' : 'text-red-600'}`}>
                                    {numVal}%
                                  </span>
                          ...
                          if (typeof value === 'object' && value !== null) {
                            displayContent = <div className="space-y-1 text-xs">
                                    {Object.entries(value as Record<string, any>).map(([key, val]) => <div key={key} className="flex justify-between gap-2">
                                        <span className="text-muted-foreground">{key}:</span>
                                        <span className="font-medium">{currency} {Number(val || 0).toLocaleString()}</span>
                                      </div>)}
                                  </div>;
                        ...
                        return <td key={carrier} className={`p-4 text-center ${colorClasses[color || 'none']}`}>
                                <div className="flex flex-col items-center gap-1">
                                  {displayContent}
                                  {note && !field.hideNote && <div className="text-xs text-muted-foreground italic mt-1 max-w-[150px]">
                                      <MarkdownContent content={note} />
                                    </div>}
                                </div>
                              </td>;
